  ...
  init(){
   ...
   actions: {
     showMenu: function(post, restaurantId){
       ...
       //Load new menu list
       this.get('ajax').ajaxRequest("/api/v1/getRestaurantMenu", "POST",
         '{"idRestaurant":"'+restaurantId+'", "type":"'+post+'"}', false).fail(function(data) {
           console.log(data);
         }).then(function(data) {
     ...
     findATable: function(idRestaurant){
        ...
        if(this.get('reservation.people') == null || this.get('tempReservationDate') == null || this.get('reservation.hour') == null){
          ...
          //Send POST request
          this.get('ajax').ajaxRequest("/api/v1/checkReservationAvailability", "POST",
            JSON.stringify(this.get('reservation')), false).fail(function(data) {
              console.log(data);
