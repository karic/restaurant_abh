  ...
  actions: {
    ...
    completeReservation: function(){
      ...
      function makeReservation(){
          //Set user details
          var requestData = '{"idRestaurant":"'+self.get('currentReservation.restaurandId')+'", "persons":"'+self.get('currentReservation.people')+'", "reservationDate":"'+self.get('currentReservation.date')+'", "reservationHour":"'+self.get('currentReservation.hour')+'"}';
          self.get('ajax').ajaxRequest("/api/v1/makeReservation", "POST", requestData, false).fail(function(data) {
      ...
      if(this.get('currentUser.userLoggedIn') == true){
          ...
          if(user.get('firstName')==null || user.get('lastName')==null || user.get('email') == null || user.get('phone') == null || user.get('password') == null || user.get('city') == null || user.get('country') == null){
            ...
            //var data = '{"firstName":"'+firstName+'","lastName":"'+lastName+'","email":"'+email+'","phone":"'+phone+'","password":"'+password+'","country":"'+country+'","city":"'+city+'"}';
              ...
              self.get('ajax').ajaxRequest("/api/v1/register", "POST", data, false).fail(function(data) {
