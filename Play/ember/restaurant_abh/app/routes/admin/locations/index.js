import Ember from 'ember';
import Filter from '../../../models/filter';
import Notification from '../../../models/notification';

export default Ember.Route.extend({
  titleToken: 'Locations / Administration',

  ajax: Ember.inject.service('ajax'),
  listItems: null,
  itemPages: [],
  currentPageNumber: 1,
  itemsPerPage:20,
  filter: Filter.create(),
  notification: Notification.create(),

  exit: function(){
    this.set('notification.visible', false);
  },
  getItems: function(){
    var self = this;

    //Set additional data
    this.set('filter.itemsPerPage', this.get('itemsPerPage'));
    this.set('filter.pageNumber', this.get('currentPageNumber'));
    var data = JSON.stringify(self.get('filter'));

    this.get('ajax').ajaxRequest("/api/v1/admin/getFilteredLocations", "POST", data, false).fail(function(data) {
      console.log(data);
    }).then(function(data) {
      //Set info about number of pages
      var numberOfPages = JSON.parse(data.numberOfPages);
      self.set('itemPages', []);
      for(var i = 1; i <= numberOfPages; i++){
        self.get('itemPages').push(i);
      }

      //Set fetched items
      self.set('listItems', data.locations);
    });
  },

  model: function(){
    var self = this;

    //Get all items
    this.getItems();

    //Scroll to top
    $("html, body").animate({ scrollTop: 0 }, 500);

    //Return model to template
    return Ember.RSVP.hash({
      listItems: self.get('listItems'),
      itemPages: self.get('itemPages'),
      filter: self.get('filter'),
      notification: self.get('notification')
    });
  },

  actions: {
    changePage: function(pageNumber){
      this.set('currentPageNumber', pageNumber);
      this.refresh();
    },
    editItem: function(id){
      this.transitionTo('admin.locations.edit', id);
    },
    deleteItem: function(id){
      var self = this;

      this.get('ajax').ajaxRequest("/api/v1/admin/deleteLocation", "POST", '{"id": '+id+'}',
        false).fail(function(data) {
          console.log(data);
      }).then(function(data) {
        //Display notification
        self.set('notification.visible', true);
        self.set('notification.classStyle', 'alert-success');
        self.set('notification.text', 'Successful remove!');
        self.refresh();
      });
    },
    searchTextChanged: function(){
      this.refresh();
    }
  }
});
