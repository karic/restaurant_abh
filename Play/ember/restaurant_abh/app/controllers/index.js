...
export default Ember.Controller.extend({
  ...
  loadNearestRestaurants: function(){
    ...
    if(self.get('model.currentLocationCoordinates').length > 0 && self.get('model.currentLocationCoordinates') != "undefined"){
      ...
      var requestData = '{"latitude":'+self.get('model.currentLocationCoordinates')[0]+', "longitude":'+self.get('model.currentLocationCoordinates')[1]+'}';

      //Return nearest restaurants
      this.get('ajax').ajaxRequest("/api/v1/getAllNearestRestaurants", "POST", requestData, false).fail(function(data) {
        console.log(data);
      }).then(function(data) {
